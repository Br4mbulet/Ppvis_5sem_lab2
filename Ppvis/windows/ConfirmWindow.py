...
class ConfirmWindow(Window):
    def __init__(self, controller, **kw):
        super(ConfirmWindow, self).__init__(controller, **kw)

        self.title_label._set_text("""
            Заказ:               Блюдо            
            Ценa:                140p            
            Адрес доставки:      улица А, дом 1            
        ...
        continue_button = Button(text="Продолжить", background_color=(0.53, 0.71, .53, .6), size_hint=(0.4, 0.2),
                             on_press=lambda x: controller.set_present_screen_name("OrderOKWindow"))
        exit_button = Button(text="Выход", background_color=(0.53, 0.71, .53, .6), size_hint=(0.4, 0.2),
